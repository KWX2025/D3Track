...
class VisAttentionmap(VisBase):
    ...
    def block_list_callback_handler(self, data):
        self.block_list[data['propertyId']]['value'] = data['value']
        self.visdom.properties(self.block_list, opts={'title': 'Attentionmap UI'}, win='attentionmap_ui')
        self.draw_data()
    ...
    def save_data(self, data, idxs, tensor_shape, mode='max', pad=0., upsample=8):
        ...
        if mode=='max':
            raw_data[idxs] = data[0].sum(0).max(-1).values
        else:
            raw_data[idxs] = data[0].sum(0).mean(-1)
            
        raw_data = F.interpolate(raw_data.reshape(tensor_shape).unsqueeze(0).unsqueeze(0), scale_factor=upsample, mode='bilinear')
        data = data.view(-1, *data.shape[-2:])
        data = data.flip(1)
        if self.block_list is None:
            self.block_list = []
            ...
            self.visdom.properties(self.block_list, opts={'title': 'Attentionmap UI'}, win='attentionmap_ui')
            self.visdom.register_event_handler(self.block_list_callback_handler, 'attentionmap_ui')

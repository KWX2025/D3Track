    ...
    for name in name_list:
        ...
        if name == "LasHeR_trainingSet" or name == 'LasHeR':
            if settings.use_lmdb:
                ...
                datasets.append(
                    LasHeR_trainingSet(settings.env.lasher_trainingset_dir, split='train', image_loader=image_loader,
                                       attr=attr))
        if name == "RGBT234":
            if settings.use_lmdb:
                print("Building lasher dataset from lmdb")
...
def build_dataloaders(cfg, settings):
    ...
    dataset_train = sampler.TrackingSampler(
        datasets=names2datasets(cfg.DATA.TRAIN.DATASETS_NAME, settings, opencv_loader, attr=attr),
                ...
                param_dicts.append(
                    {
                        "params": [p for n, p in net.named_parameters() if
                                   fintune_weight_key(n, k) and p.requires_grad],
                        "lr": lr,
                    }
                )

...
from lib.train.actors import OSTrack_DFTrack_Actor
# for import modules
import importlib
...
def run(settings):
    ...
    if settings.local_rank != -1:
        ...
        if dist.is_initialized():
            ...
            net = net.to(torch.device(f"cuda:{settings.local_rank}" if torch.cuda.is_available() else "cpu"))
    ...
    if settings.script_name in ["dftrack"]:  #
        focal_loss = FocalLoss()
        objective = {'giou': giou_loss, 'l1': l1_loss, 'focal': focal_loss, 'cls': BCEWithLogitsLoss()}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT,
                       'style': cfg.TRAIN.STYLE_DISTILL_WEIGHT,
                       'content': cfg.TRAIN.CONTENT_DISTILL_WEIGHT, 'focal': 1., 'cls': 1.0}
        actor = OSTrack_DFTrack_Actor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
